...
def mnist_real3(out='./', prefix='MNIST_'):
...
def mnist_real4(out='./', prefix='MNIST_'):
    mnist = utils.load_mnist(val_size=0)
    tr_labels = []
    for i in mnist.train.labels:
...
def cifar10_real3(out='./', prefix='CIFAR10_'):
    from keras.datasets import cifar10
    (train_x, train_y), (test_x, test_y) = cifar10.load_data()
...
def cifar10_real4(out='./', prefix='CIFAR10_'):
    from keras.datasets import cifar10
    (train_x, train_y), (test_x, test_y) = cifar10.load_data()
...
def cifar10_real5(out='./', prefix='CIFAR10_'):
    from keras.datasets import cifar10
    (train_x, train_y), (test_x, test_y) = cifar10.load_data()
...
def ng20_real5(out='./', prefix='20NG_'):
    try:
...
def ng20_real6(out='./', prefix='20NG_'):
    try:
        ng20 = np.load(out + prefix + 'w2v.npz')
    except:
        print out + prefix + 'w2v.npz not found, run  ./mk.py mk_20newsgroups_avgw2v'
...
def mk_reuters_data(data_dir='./'):
    ...
    np.save('perm.npy', p)
    mk_reuters_sublabels()

    return data, target, ids



def mk_reuters_sublabels(data_dir='./'):
    from os.path import join
    import pickle
...
def mk_reuters_subset(out='./', prefix='REU_'):
    import pickle
    np.random.seed(1234)
...
def reuters_real5(out='./', prefix='REU_'):
    try:
